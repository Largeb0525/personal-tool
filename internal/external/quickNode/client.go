...
func CreateFreezeTx(req FreezeRequest) (tx Transaction, err error) {
	...
	resp, err := httpclient.DefaultClient.Do(httpReq)
	if err != nil {
...
func CreateUnfreezeTx(req UnfreezeRequest) (tx Transaction, err error) {
	body, err := json.Marshal(req)
	if err != nil {
		return tx, fmt.Errorf("json marshal failed: %w", err)
	}
	url := fmt.Sprintf(unfreezeBalanceURL, AppID)
	httpReq, err := http.NewRequest("POST", url, bytes.NewBuffer(body))
	if err != nil {
		return tx, fmt.Errorf("create request error: %w", err)
	}
